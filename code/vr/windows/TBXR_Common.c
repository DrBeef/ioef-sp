/*
===========================================================================
TBXR_Common.c -- Team Beef OpenXR common layer (Windows/PCVR).

Ported/adapted from RealRTCWXR (code/RealRTCWXR/RealRTCWXR/windows/
TBXR_Common.c) for the ioEF Elite Force VR port (milestone M1).

Engine-agnostic: this file includes only VrCommon.h (which pulls in
q_shared/qcommon and the OpenXR/GL headers) and SDL.  Anything that would
need engine client state (clc/cls/re) is delegated to small glue functions
implemented in EFXR_SurfaceView.c (EFXR_GetScreenResolution / EFXR_SwapWindow).

RTCW-specific bits dropped for M1:
 - vr.cgzoommode zoom-fov adjustment in submitFrame (field not in the
   VrClientInfo.h contract).
 - The real controller/haptics implementations (TBXR_InitActions /
   TBXR_UpdateControllers / TBXR_ProcessHaptics / TBXR_Vibrate) live in
   RealRTCW's OpenXrInput.c, which is not ported in M1; stubs are provided
   here so the layer links.
===========================================================================
*/
